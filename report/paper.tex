...
\author{Vineeth Animireddy\\ \small Independent Researcher}
\date{August 2025}

% ---- Safe figure macro: compiles even if file is missing ----
\newcommand{\safeincludegraphics}[2]{% #1=path #2=caption
  \begin{figure}[h]
    \centering
    \IfFileExists{#1}{%
      \includegraphics[width=0.9\linewidth]{#1}%
    }{%
      \fbox{\parbox{0.88\linewidth}{\centering \textbf{Figure not available}\\\texttt{#1}}}%
    }%
    \caption{#2}
  \end{figure}
}
...
We report aggregate metrics (no raw generations). Charts below embed the latest PNGs from the repository if present.

\safeincludegraphics{results/tiny_snapshot_summary.png}{Tiny Model Safety Snapshot (daily).}
\safeincludegraphics{results/tiny_live_summary.png}{Live Evaluation Snapshot (distilgpt2 via API, daily).}
\safeincludegraphics{results/safety_timeseries.png}{Safety Timeseries (daily class rates over time).}
\safeincludegraphics{results/drift_index.png}{Drift Index (JSD vs 7-day average).}
